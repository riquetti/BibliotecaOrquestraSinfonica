...
def minha_view(request):
    categorias_unicas = Livros.objects.values_list('categoria', flat=True).distinct()
    print(categorias_unicas)
    return render(request, 'home.html', {'categorias_unicas': categorias_unicas})
...
def cadastrar_livro(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            novo_livro = form.save(commit=False)  # Não salvar no banco de dados ainda
            novo_livro.localizacao = form.cleaned_data['localizacao']
            novo_livro.exemplares_disponiveis = form.cleaned_data['exemplares_disponiveis']
            novo_livro.save()  # Agora sim, salvar no banco de dados
